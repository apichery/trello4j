...
public class TrelloImpl implements Trello {
     ...
     */
    @Override
    public List<Action> getActionsByCard(final String cardId, Map<String, Object> params) {
        validateObjectId(cardId);
    ...
    @Override
    public Attachment getCoverByCard(final String cardId) {
        validateObjectId(cardId);
        ...
        InputStream jsonContent = doGet(url);
        JsonElement jsonElement = trelloObjFactory.unmarshallToJson(jsonContent);
        if (jsonElement.isJsonArray()) {
            List<Attachment> result = trelloObjFactory.unmarshallToObj(new TypeToken<List<Attachment>>() {
            }, jsonElement);
            ...
            // Handle the case where trello return an invalid object: {"_value": "null"}
            JsonObject jsonObj = jsonElement.getAsJsonObject();
            if (jsonObj != null && jsonObj.get("_value") != null && jsonObj.get("_value").isJsonNull()) {
                return null;
            }
            return trelloObjFactory.unmarshallToObj(new TypeToken<Attachment>() {
            }, jsonElement);
        }
    }
    ...
    private InputStream doRequest(String url, String requestMethod) {
        while (true) {
            ...
            } catch (TrelloRateLimitExceedException e) {
                // Retry
                try {
    ...
    private InputStream doRequest(String url, String requestMethod, Map<String, String> map) {
        try {
            ...
            if (conn.getResponseCode() == 429) {
                throw new TrelloRateLimitExceedException();
